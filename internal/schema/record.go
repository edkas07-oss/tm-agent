...
// NewEventRecord creates a new canonical EventRecord.
func NewEventRecord(eventType, targetID string, generation int, status, strength string, value interface{}, observedAt time.Time) *EventRecord {
	gen := generation
	return &EventRecord{
		SchemaVersion: 1,
		Type:          eventType,
...
// MarshalIndent formats the record into indented JSON bytes.
func (r *EventRecord) MarshalIndent() ([]byte, error) {
	return json.MarshalIndent(r, "", "  ")
}

// Marshal formats the record into standard JSON bytes.
